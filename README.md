...
#### Base Model
We assume the base model to be present in line with the huggingface repository strucuture, i.e. [llama3-instruct-8b](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct). We refer to a base model's repositories’ local path by **base_model_dir**. If you have not done so, create a such a local copy using git clone. 
